...
def init_db():
    ...
    # Ustalar / Taşeronlar tablosu (Sıvacı, Kalıpçı vb.)
    cursor.execute('''
...
if st.session_state.user_id is None:
    ...
    with st.sidebar.expander("➕ Yeni Proje Ekle"):
        ...
        if st.button("Proje Oluştur"):
            if new_proj_name:
                run_query("INSERT INTO projects (user_id, name) VALUES (?, ?)", (st.session_state.user_id, new_proj_name))
    ...
    if not project_dict:
        ...
        tab_homeowners, tab_tradesmen, tab_summary = st.tabs(["🏠 Ev Sahipleri & Gelirler", "👷 Ustalar & Taşeronlar", "📊 Genel Durum"])
        ...
        with tab_tradesmen:
            ...
            with st.form("new_tradesman"):
                ...
                if submit_t and t_name:
                    run_query("INSERT INTO tradesmen (project_id, name, trade_type) VALUES (?, ?, ?)", (project_id, t_name, t_type))
                    st.success("Usta kartı açıldı!")
                    st.rerun()
            
            st.divider()
            tradesmen_list = run_query("SELECT id, name, trade_type FROM tradesmen WHERE project_id = ?", (project_id,), fetch=True)
            ...
            if tradesmen_list:
                ...
                with st.form("t_trans_form"):
                    ...
                    t_date = st.date_input("İşlem Tarihi", datetime.now(), key="t_date")
                    t_amount = st.number_input("Tutar (TL)", min_value=0.0, step=500.0, key="t_amt")
                    t_desc = st.text_input("Açıklama (Örn: 1. kat sıva hakedişi / Nakit avans)", key="t_desc")
                    ...
                    if sub_tr and t_amount > 0:
                        run_query("INSERT INTO tradesman_transactions (tradesman_id, trans_type, date, amount, description) VALUES (?, ?, ?, ?, ?)",
                                  (t_id, ttype, str(t_date), t_amount, t_desc))
        ...
        # --- GENEL DURUM SEKMESİ ---
        with tab_summary:
            st.subheader(f"📊 {selected_project_name} Proje Özeti")
            
            all_cust = run_query("SELECT id FROM customers WHERE project_id = ?", (project_id,), fetch=True)
            total_collected = 0
            total_receivable = 0
            for c in all_cust:
                cid = c[0]
                tp = run_query("SELECT total_price FROM customers WHERE id = ?", (cid,), fetch=True)[0][0]
            ...
            col1, col2 = st.columns(2)
            col1.metric("Toplam Tahsil Edilen Para", f"{total_collected:,.2f} TL")
            col1.metric("Ev Sahiplerinden Kalan Alacak", f"{total_receivable:,.2f} TL")
            
            col2.metric("Ustalar Ödenen Toplam", f"{total_tradesman_paid:,.2f} TL")
            col2.metric("Ustaların Kalan Toplam Alacağı", f"{total_tradesman_debt - total_tradesman_paid:,.2f} TL")
